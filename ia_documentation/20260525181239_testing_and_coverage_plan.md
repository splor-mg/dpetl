...
There are two practical ways to show coverage in the README.

### Recommended: Use Codecov

This is the best option for a GitHub project because the badge updates
automatically after CI runs.

Requirements:

- Generate `coverage.xml` during tests:

```bash
pytest --cov=dpetl --cov-report=term-missing --cov-report=xml
```

- Add a GitHub Actions workflow that runs tests and uploads `coverage.xml` to
  Codecov.
- Add a README badge like:

```markdown
[![codecov](https://codecov.io/gh/gabrielbdornas/dpetl/graph/badge.svg)](https://codecov.io/gh/gabrielbdornas/dpetl)
```

Depending on the repository visibility and Codecov configuration, a token may
be needed in GitHub repository secrets.

### Alternative: Commit a Generated SVG Badge

This avoids an external service, but the badge only updates when someone
...
This is simpler locally, but less reliable because it can become stale.
...
5. Decide whether `pre_test` and `post_test` should stay automatic or become
   explicit tasks.
6. Add a CI workflow that publishes coverage.
7. Add the coverage badge to README.
