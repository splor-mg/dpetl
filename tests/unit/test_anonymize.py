...
def test_apply_anonymization_with_filter_condition():
    ...
    result = anonymize.apply_anonymization(field, table, None)
    data = list(result)
    # Row 1: condition true -> should be anonymized
    assert data[1][0] != 'value1'
    assert len(data[1][0]) == 16
    # Row 2: condition false -> should stay original
    assert data[2][0] == 'value2'


def test_apply_anonymization_with_filter_eval_error():
    """Test that invalid filter expression returns original value."""
    field = type('Field', (), {
        'name': 'col1',
        'missing_values': [],
        'custom': {'anonymize': {'method': 'sha256', 'filter': 'invalid syntax'}}
    })()
    table = etl.wrap([['col1'], ['value1']])
    result = anonymize.apply_anonymization(field, table, None)
    data = list(result)
...
def test_build_constraints_mask_with_single_variant():
    """Test build_constraints for a single mask variant with # and *."""
    field = type('Field', (), {
...
def test_build_constraints_literal_mask():
    """Test build_constraints for a literal mask like [INFO]."""
    field = type('Field', (), {
    ...
    constraints = anonymize.build_constraints(field)
    pattern = constraints['pattern']
    assert pattern == r'^INFORMACAO\ COM\ RESTRICAO\ DE\ ACESSO$'
...
def test_build_constraints_literal_with_spaces():
    """Test build_constraints for a literal mask with spaces."""
    field = type('Field', (), {
        'custom': {'anonymize': {'method': '[INFORMACAO COM RESTRICAO DE ACESSO]'}},
        'constraints': {}
    })()
